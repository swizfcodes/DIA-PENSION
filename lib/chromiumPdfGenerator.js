...
class ChromiumPDFGenerator {
  constructor() {
    ...
    this.logFile = '/home/evenplai/backend/chromium-debug.log';
  }
 ...
 async ensureChromiumExists() {
  const chromiumPath = '/home/evenplai/tmp/.chromium-temp/chromium';
  ...
  try {
    ...
    const tempDir = '/home/evenplai/tmp/.chromium-temp';
    ...
    // Force re-extraction
    const newPath = await this.chromium.executablePath({ cacheDir: chromiumExtractDir });
  ...
  async _initializeProduction() {
    try {
      ...
      process.env.HOME = '/home/evenplai';
      ...
      try {
        ...
        this.log(`⚠️ executablePath with cacheDir failed: ${pathError.message}`);
  ...
  async generateFromHTML(htmlContent, options = {}) {
    ...
    const htmlSizeInMB = Buffer.byteLength(htmlContent, 'utf8') / (1024 * 1024);
    if (htmlSizeInMB > 10) {
      throw new Error(`HTML too large: ${htmlSizeInMB.toFixed(2)}MB. Maximum is 10MB.`);
    }
